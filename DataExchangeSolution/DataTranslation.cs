            ...
            if (info.InputFormat != 0 && info.OutputFormat != 0 && info.InputFormat != info.OutputFormat)
            {
                ...
                if(path.Count()>0)
                {
                    ...
                    for(var i=0;i<path.Count();i++)
                    {
                        ...
                        formats.Add(path[i]);
                        Program.logger.log("Starting conversion from " + formats.ElementAt(i) + " to " + formats.ElementAt(i + 1) + " requested by: " + enpointProperty.Address);
                        try
                        {
                    ...
                    {
                        Program.logger.log("Conversion requested by: " + enpointProperty.Address + " failed");
                    }
